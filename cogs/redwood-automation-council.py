...
class CouncilCog(commands.Cog, name="Council Commands Cog"):
    ...
    async def council(self, ctx: commands.Context) -> None:
        ...
        embed.add_field(name="Mayor", value=guild.get_role(646549322682466305).members[0].mention if len(ctx.guild.get_role(646549322682466305).members) > 0 else "VACANT", inline=False)
        embed.add_field(name="Deputy Mayor", value=guild.get_role(646551227626160139).members[0].mention if len(ctx.guild.get_role(646551227626160139).members) > 0 else "VACANT", inline=True)
        embed.add_field(name="Council Chairperson", value=guild.get_role(673008336010084378).members[0].mention if len(ctx.guild.get_role(673008336010084378).members) > 0 else "VACANT", inline=False)
        embed.add_field(name="City Council Members", value="\n".join([member.mention for member in guild.members if discord.utils.get(member.roles, id=646549329493884929) and not discord.utils.get(member.roles, id=673008336010084378)]), inline=True)
